...
import { formatFlattenedErrors, phoneRegex } from "../utils";
...
const panValidator = z
	.string()
	.trim()
	.toUpperCase()
	.regex(/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/, "Invalid PAN number");

const ifscValidator = z
	.string()
	.trim()
	.toUpperCase()
	.regex(/^[A-Za-z]{4}[a-zA-Z0-9]{7}$/, "Invalid IFSC code");

const accountNumberValidator = z
	.string()
	.trim()
	.min(9, "Account number must have at least 9 digits")
	.max(18, "Account number cannot have more than 18 digits");

const accountHolderValidator = z
	.string()
	.trim()
...
const createVendorWithCashfreeSchema = z.object({
	...
	bankAccount: z.object({
		...
		accountHolder: accountHolderValidator,
		ifsc: ifscValidator,
	}),
	kyc: z.object({
		pan: panValidator,
